...
MVC separates the internal representation of information from the way that information is presented to the end user. This separating of internal representation and ist presentation allows you to change any of these without affecting the other. An application structured according to MVC separates its functionality into the three areas described below.
...
```bash
  rails generate controller posts new destroy show index
```
...
Try generating a scaffold and controller (with different names) and provide the controller with some actions as arguments. Have a look at the difference between the generated code, why do you think this is? Discuss
...
Start by designing an appropriate json structure for the a blog post, including the category it belongs to and associated attributes. You must then override the `to_json` method. You should build a nested hash in this method and call `to_json` on it. An example is shown below:
